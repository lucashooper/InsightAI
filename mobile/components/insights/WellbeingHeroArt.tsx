...
export const WELLBEING_ART_SIZE = 110;

type Props = {
  tier: WellbeingTier;
  /** Card surface colour — fade overlays blend into this. */
  cardBg: string;
  isDark?: boolean;
};

/** Bottom-right illustration with diagonal mask + soft edge fades. */
export default function WellbeingHeroArt({ tier, cardBg, isDark = false }: Props) {
  ...
  const edgeFade = isDark ? 'rgba(16,16,22,1)' : cardBg;
...
const styles = StyleSheet.create({
  wrap: {
    ...
    bottom: 0,
    right: 0,
    width: WELLBEING_ART_SIZE,
    height: WELLBEING_ART_SIZE,
    zIndex: 0,
    overflow: 'hidden',
  },
  maskHost: {
